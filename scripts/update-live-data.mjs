...
async function main() {
  ...
  const power = await updateSource({
    ...
    fallback: fallbackCard(
      'Не вдалося автоматично оновити графік відключень',
      'Джерело тимчасово не віддало дані або було заблоковане захистом сайту. Подивіться log-файл поруч.',
      POWER_URL
    )
  }, debug);
  ...
  const alert = await updateSource({
    ...
    parser: parseAlert,
    fallback: fallbackCard(
      'Не вдалося автоматично оновити тривогу',
      'Подивіться log-файл поруч, щоб побачити причину помилки.',
      ALERT_URL
    )
  }, debug);
  ...
  const water = await updateSource({
    ...
    parser: parseWater,
    fallback: fallbackCard(
      'Не вдалося автоматично оновити рівень води',
      'Подивіться log-файл поруч, щоб побачити причину помилки.',
      WATER_URL
    )
  }, debug);
  ...
  const fishing = await updateSource({
    ...
    parser: parseFishing,
    fallback: fallbackCard(
      'Не вдалося автоматично оновити прогноз рибалки',
      'Подивіться log-файл поруч, щоб побачити причину помилки.',
      FISHING_URL
    )
  }, debug);
